...
var BasicInfo={
    ...
    deleteBasicInfo: function(id,callback){
        return db.query('delete from basicinfo where emp_id=?',[id],callback);
    },
  
    updateBasicInfo: function(eid,basicinfo,callback){
        return db.query('update basicinfo set EmpNo=?, Title=?,Initial=?,FirstName=?,MiddleName=?,LastName=?,Gender=?,DOB=?,EmpAge=?,OPhone=?,PMobile=?,OMail=?,PMail=?,PMail2=?,PMail3=?,Photo=?,Birth=?,Domicile=?,Religion=?,Caste=?,Nationality=?,Voter=?,PanNo=?,Adhar=?,Status=?,Child=?,MDate=?,SName=?,BankName=?,AccntType=?,Payment=?,AccntNo=?,BankDetail=?,IFSC=?,RBankNm=?,RAccntNo=? where emp_id=?',
        [basicinfo.EmpNo, basicinfo.Title, basicinfo.Initial, basicinfo.FirstName, basicinfo.MiddleName,basicinfo.LastName,
